...
export default function FluidPage() {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            ...
            httpUtil.get(httpUtil.path.fluids, headers)
                ...
                .catch(e => console.error("Fetch fluids failed", e));
        }, 1000);
    ...
    const handleCraftRequest = useCallback((itemStack) => {
        ...
        setCraftAmount(1000); // Default 1000mB for fluids
        setSelectedCpu(null);
        fetchCpus();
    ...
    const submitCraft = () => {
        ...
        CommandUtil.submitCommand("requestItem", JSON.stringify(payload, null, 2), async resp => {
            if (resp.status === 200) {
                message.success(`已请求制造 ${craftAmount} mB ${selectedItem.label}`);
    ...
    return (
        <Card style={{ minHeight: '100%' }}>
            <Space style={{ marginBottom: 16, flexWrap: 'wrap' }}>
                <Button danger icon={<DeleteOutlined />} onClick={handleClearItems}>清理所有流体</Button>
                <Button type="primary" icon={<CloudSyncOutlined />} onClick={handleRefreshStorage}>搜寻流体</Button>
            </Space>
            ...
            <List
                ...
                locale={{ emptyText: <Empty description="暂无流体数据" /> }}
                renderItem={(item) => (
                    <List.Item>
